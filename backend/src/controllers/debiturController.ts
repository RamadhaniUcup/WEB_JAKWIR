 ...
 */
export async function getDebiturProfile(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
 ...
 */
export async function updateDebiturProfile(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const idDebitur = req.user?.id;
    if (!idDebitur) {
      res.status(401).json({ message: "Unauthorized. Pengguna tidak teridentifikasi." });
    ...
    if (namaDebitur !== undefined) updateData.namaDebitur = namaDebitur;
