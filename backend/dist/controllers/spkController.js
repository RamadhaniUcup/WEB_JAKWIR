 ...
 */
async function processSpkDecision(idPengajuan, idPenyediaJasaAdmin) {
    ...
    await prisma.pengajuan.update({
        where: { idPengajuan },
        data: { statusPeminjaman: statusPeminjaman }
    });
    return {
        idPengajuan,
        ...scores,
 ...
 */
export async function calculateBulkSpk(req, res) {
    try {
        const idPenyediaJasaAdmin = req.user?.idPenyediaJasa;
        if (!idPenyediaJasaAdmin) {
 ...
 */
export async function getLaporanSpk(req, res) {
    try {
        const idPenyediaJasaAdmin = req.user?.idPenyediaJasa;
        if (!idPenyediaJasaAdmin) {
