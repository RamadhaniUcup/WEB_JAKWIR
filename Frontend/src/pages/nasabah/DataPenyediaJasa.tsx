...
import { useGetPenyediaJasaList } from "../../hooks/useApi.js";
import { GlassCard } from "../../components/common/glasscard.js";

interface LivePenyediaJasa {
...
export const DataPenyediaJasa: FC = () => {
  const { data: penyediaJasaList, isLoading, error } = useGetPenyediaJasaList();

  return (
    <div className="p-6 max-w-7xl mx-auto text-slate-200 pt-32 font-sans">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-white">Mitra Penyedia Jasa</h1>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-xs text-slate-400 mt-3 font-semibold">Mengambil daftar mitra penyedia jasa...</p>
        </div>
      ) : error ? (
        <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-semibold rounded-xl text-center">
          Gagal mengambil data penyedia jasa. Silakan muat ulang halaman.
        </div>
      ) : penyediaJasaList && penyediaJasaList.length === 0 ? (
        <div className="p-4 bg-white/5 border border-white/10 text-slate-400 text-sm font-semibold rounded-xl text-center">
          Tidak ada lembaga penyedia jasa aktif terdaftar saat ini.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {penyediaJasaList?.map((k: LivePenyediaJasa, idx: number) => {
            // Masing-masing penyedia jasa memiliki tipe/limit tiruan untuk melengkapi visual
            ...
            return (
              <GlassCard key={k.idPenyediaJasa} className="p-6 flex flex-col justify-between" hoverEffect>
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-lg text-white">{k.namaPenyediaJasa}</h3>
...
export default DataPenyediaJasa;
